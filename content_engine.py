...
class ContentEngine:
    def __init__(self, papers_path='papers.csv', embeddings_path='embeddings.npy', model_name='all-mpnet-base-v2'):
        """
        初始化内容引擎：加载数据和模型。
        """
        print("正在加载数据和模型，请稍候...")
        ...
        if not os.path.exists(papers_path):
            raise FileNotFoundError(f"未找到文件: {papers_path}")
        self.df = pd.read_csv(papers_path)

        # 2. 加载 Embeddings 矩阵
        if not os.path.exists(embeddings_path):
            raise FileNotFoundError(f"未找到文件: {embeddings_path}")
